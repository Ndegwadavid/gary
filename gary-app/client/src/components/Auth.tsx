...
import { Mail, Lock, X, AlertCircle, LogIn, Key } from "lucide-react"
...
const Auth: React.FC<AuthProps> = ({ onClose }) => {
  ...
  const [resetEmail, setResetEmail] = useState("") // New state for reset email input
  const [resetMessage, setResetMessage] = useState<string | null>(null) // New state for reset feedback
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      showForgotPassword ? handlePasswordReset() : handleEmailAuth()
    }
  }
      ...
      <Card className="w-full max-w-md z-10 shadow-lg border-primary/10">
        <CardHeader className="relative">
          ...
          <CardDescription>
            {showForgotPassword
              ...
              : isSignUp
              ? "Sign up to start sharing music with friends"
          ...
          {resetMessage && showForgotPassword && (
            <div
              className={`p-3 rounded-md flex items-start ${
                resetMessage.includes("sent") ? "bg-green-500/10 text-green-500" : "bg-destructive/10 text-destructive"
              }`}
            >
              <AlertCircle className="h-5 w-5 mr-2 mt-0.5 flex-shrink-0" />
        ...
        <CardFooter className="flex flex-col">
          {!showForgotPassword ? (
            ...
            <p className="text-sm text-center text-muted-foreground">
              <Button variant="link" className="p-0 h-auto font-normal" onClick={() => setShowForgotPassword(false)}>
                Back to Login
              </Button>
            </p>
          )}
